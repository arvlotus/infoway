...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES["image"]["size"] > 0) {
        ...
        $targetDir = "../../src/img/icons";  // Substitua pelo diretório correto
        ...
        if ($_FILES["image"]["error"] !== UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
                $image_path = "src/img/icons" . basename($_FILES["image"]["name"]);
    ...
    header("Location: ../edit_icon.php?icon_id=$icon_id");
    exit();
}
?>
